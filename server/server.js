...
const tradeInsightsService = require("./services/tradeInsights");
...
app.get(
  ...
  async (req, res) => {
    ...
    const insights = await tradeInsightsService.getInsights(
      platformAccountIds,
      options
    );
...
app.post(
  ...
  async (req, res) => {
    const { accountId } = req.params;
    const { platformAccountId, timezone, timezoneOffset } = req.body;
