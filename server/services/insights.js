...
async function getTradesPnL(platformAccountIds, options = {}) {
  ...
  console.log("groupedTrades", JSON.stringify(groupedTrades));

  const { dailyPnL, cummulativePnL } = Array.from(
    groupedTrades.values()
  ).reduce(
    (acc, trades) => {
      trades.forEach((trade) => {
        const { date, securityType, quantity, openPrice, closePrice, pnl } =
          trade;

        if (pnl !== null) {
          console.log("pnl", pnl);
          console.log("pnl type", typeof pnl);
          acc.dailyPnL.push({ date, pnl });
          const cummPnL =
            (acc.cummulativePnL[acc.cummulativePnL.length - 1]? acc.cummulativePnL[acc.cummulativePnL.length - 1].pnl : 0) + pnl;
          acc.cummulativePnL.push({ date, pnl: cummPnL });
        } else {
          let pnl = (closePrice - openPrice) * quantity;
          ...
          acc.dailyPnL.push({ date, pnl });
          const cummPnL =
            (acc.cummulativePnL[acc.cummulativePnL.length - 1] || 0) + pnl;
          acc.cummulativePnL.push({ date, pnl: cummPnL });
        }
      });
    ...
    { dailyPnL: [], cummulativePnL: [] }
  );

  let filteredDailyPnL = dailyPnL;
  let filteredCummulativePnL = cummulativePnL;

  if (fromDate) {
    filteredDailyPnL = filteredDailyPnL.filter(
      (trade) => new Date(trade.date) >= fromDate
    );
    filteredCummulativePnL = filteredCummulativePnL.filter(
      (trade) => new Date(trade.date) >= fromDate
    );
  }
  ...
  console.log("filteredDailyPnL", JSON.stringify(filteredDailyPnL));

  return {
    dailyPnL: filteredDailyPnL,
    cummulativePnL: filteredCummulativePnL,
  };
}
